...
for file_name in file_list:
    ...
    data["global_rank"] = (
        data.groupby(["site", "keyword"])["page"].transform(
    ...
    for site in sites:
        ...
        pivoted = site_data.pivot_table(
            index=["job ad id"], columns="keyword", values="global_rank"
        )

        if pivoted.empty or pivoted.isnull().all().all():
            print(
                f"skipping site '{site}' in file '{file_name}' due to lack of valid data."
            )
            continue
        ...
        spearman_file = os.path.join(
            SPEARMAN_PATH, f"{site}_{job_group}-spearman_correlation.png"
        )
        ...
        kendall_file = os.path.join(
            KENDAL_PATH, f"{site}_{job_group}-kendall_correlation.png"
        )
        ...
        for i, term1 in enumerate(search_terms):
            for term2 in search_terms[i + 1 :]:
                ...
                scatter_file = os.path.join(
                    SCATTER_PATH, f"{site}_{job_group}-{term1}_vs_{term2}_scatter.png"
                )
